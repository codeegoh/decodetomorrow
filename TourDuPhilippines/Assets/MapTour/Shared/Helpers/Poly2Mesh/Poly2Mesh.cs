...
//		Profiler.Exit(profileID);
	...
	/// <summary>
	/// Create data for a Mesh using a background thread for the non-unity part of the job.
	/// </summary>
	/// <returns>The freshly minted mesh.</returns>
	/// <param name="polygon">Polygon you want to triangulate.</param>
	public static GOMesh CreateMeshInBackground(Polygon polygon) {
		...
		if (polygon.holes.Count == 0 && (polygon.outside.Count == 3
			|| (polygon.outside.Count == 4 && polygon.outside[3] == polygon.outside[0]))) {
			return CreateTriangleInBackground(polygon);
			return null;
		}
		...
		preMesh.vertices = vertexList.ToArray();
...
//		Profiler.Exit(profileID);
	...
	/// <summary>
	/// Create data for a Mesh in background containing just the FIRST triangle in the given Polygon.
	/// (This is a much easier task since we can skip triangulation.)
	/// </summary>
	/// <returns>The freshly minted mesh.</returns>
	/// <param name="polygon">Polygon you want to make a triangle of.</param>
